...
train_test_split = load_component(source=str(Path("..", "components", "train_test_split.yml")))
model_selection = load_component(source=str(Path("..", "components", "model_selection.yml")))
evaluate_model = load_component(source=str(Path("..", "components", "evaluate_model.yml")))
...
pipeline_job = template_pipeline(
    dataset_input_path=Input(type=AssetTypes.URI_FILE, path="azureml://datastores/workspaceblobstore/paths/template_data/training/iris.parquet"),
    train_size=0.8,
    num_folds=3
)
